...
def load():
    if platform.system() == "Windows":
        return CDLL("DobotDll/DobotDll.dll",  RTLD_GLOBAL)
    elif platform.system() == "Darwin" :
        return CDLL("DobotDll/libDobotDll.dylib",  RTLD_GLOBAL)
    else:
...
def ConnectDobot(api, portName,  baudrate):
    ...
    :param portName: 端口号
    :param baudrate: 波特率
    :return: DobotConnect_NoError：连接 Dobot 控制器成功
             DobotConnect_NotFound：未找到 Dobot 控制器接口
             DobotConnect_Occupied：Dobot 控制器接口被占用
    """
